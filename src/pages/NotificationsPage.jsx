...
import { InboxActivityView } from '@/features/inbox/components/InboxActivityView';
...
import { useInboxPreferencesStore } from '@/features/inbox/inboxPreferencesStore';
...
function ComposeQueryModal({ open, onClose }) {
  ...
  const onSubmit = (values) => {
    const payload = { subject: values.subject, body: values.body, type: 'query' };
    if (values.recipientType === 'person' && values.to) payload.to = values.to;
    else if (values.recipientType === 'department' && values.department) payload.department = values.department;
    else if (values.recipientType === 'team' && values.team) payload.team = values.team;
  ...
  const handleCreateTask = () => {
    ...
    createTask.mutate(
      { id: message._id, projectId, title: message.subject },
      {
        onSuccess: (task) => {
          onClose();
          if (task?.project) navigate(`/projects/${task.project}`);
        },
      }
    );
  };
    ...
    <Modal open onClose={onClose} title={message.subject}>
      <div className="space-y-4">
        <div className="text-sm text-graphite">
          <p>{isIncoming ? 'From' : 'To'}: <span className="font-medium text-ink">{isIncoming ? message.from?.name : message.to?.name}</span></p>
          <p className="mt-1">{formatDistanceToNow(new Date(message.createdAt), { addSuffix: true })}</p>
          {message.department?.name && <p className="mt-1">Department: {message.department.name}</p>}
        ...
        {showCreateTask ? (
          <div className="space-y-3 rounded-lg border border-hairline bg-cloud p-3">
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => setShowCreateTask(false)}>Cancel</Button>
              <Button type="button" size="sm" disabled={!projectId || createTask.isPending} onClick={handleCreateTask}>
...
function QueriesPanel() {
  ...
  const { data: inboxData, isLoading } = useMessageInbox({ limit: 50 });
  ...
  const openMessage = (message, { createTask = false } = {}) => {
    ...
    if (isMessageRecipient(message) && !message.isRead) markMessageRead.mutate(message._id);
  };
    ...
    <>
      <div className="mb-4 flex flex-wrap justify-end gap-2">
        <Button variant="outline" onClick={() => setComposeOpen(true)}>
          <Plus className="h-4 w-4" /> Compose query
        </Button>
        {messageUnread > 0 && (
          <Button variant="outline" onClick={() => markAllMessagesRead.mutate()} disabled={markAllMessagesRead.isPending}>
      ...
      {messages.length === 0 ? (
        <EmptyState
          ...
          action={<Button onClick={() => setComposeOpen(true)}><Plus className="h-4 w-4" /> Compose query</Button>}
        />
      ) : (
        <ul className="space-y-3">
          {messages.map((m) => {
            ...
            return (
              <li key={m._id}>
                <Card className={cn('transition-shadow hover:shadow-md', isUnread && 'border-primary-soft bg-primary-soft/30')}>
                  <CardContent className="flex items-start justify-between gap-4 py-4">
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-2">
                      ...
                      <Button type="button" size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); openMessage(m, { createTask: true }); }}>
      ...
      {selectedMessage && (
        <MessageDetail
          ...
          onClose={() => { setSelectedMessage(null); setOpenCreateTask(false); }}
...
export default function NotificationsPage() {
  ...
  const { data: inboxData } = useMessageInbox({ limit: 1 });
  const chatUnread = chatData?.unread ?? 0;
  const queryUnread = inboxData?.unread ?? 0;
